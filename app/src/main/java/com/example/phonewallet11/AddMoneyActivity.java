...
//    4.在onPostExecute()方法中，设置下载完成状态时UI组件的属性
    ...
    public void btOk(View v){
        ...
        if((etmoney.getText().toString()).equals("")){
            ...
            if(money>0) {
                this.rechage(money, card, password);
//              //  连接数据库
...
//                }
    ...
    public void rechage(double money, String cardNo, String password) {
        // 充值
        ...
        String url = new ApiBaseUrl().assemblyUrl("recharge/");
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                ...
                final Recharge rechargeResult = gson.fromJson(response.body().charStream(), Recharge.class);
                Integer code = rechargeResult.getCode();
                if (!code.equals(200)) {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(AddMoneyActivity.this, rechargeResult.getMessage(),
                                    Toast.LENGTH_SHORT).show();
                        }
                    });
                } else {
                    final Data data = rechargeResult.getData();
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(AddMoneyActivity.this, "充值成功！", Toast.LENGTH_SHORT).show();
            ...
            public void onFailure(@NonNull Call call, @NonNull IOException e) {
                Toast.makeText(AddMoneyActivity.this, e.toString(), Toast.LENGTH_SHORT).show();
            }
        });
    }
}
